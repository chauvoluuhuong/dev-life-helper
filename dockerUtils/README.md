...
main() { # Check if Docker is running
...
I've created a comprehensive Docker management CLI

# Docker Manager CLI

A command-line interface tool for managing Docker resources including containers, images, volumes, and docker-compose projects.

## Features

- **Docker Compose Management**: List and run docker-compose files with various options (up, down, restart)
- **Image Management**: View all Docker images
- **Container Management**:
  - List all containers with their status
  - Stop or remove individual containers
  - Stop all containers
...
- **Resource Cleanup**:
  ...
  - Remove resources related to a specific docker-compose project
  - Remove unused resources (dangling images, stopped containers, unused volumes/networks)
...
2. Optionally, create an alias for easier access:

```bash
echo "alias docker-manager='$(pwd)/docker-manager.sh'" >> ~/.bashrc
source ~/.bashrc
```

## Usage

Run the script from the dockerUtils directory:

```bash
./docker-manager.sh
```

Or if you've created an alias:
...
- Bash shell
- Optional: `yq` command for better docker-compose project name detection

## Navigation

The tool provides an interactive menu system. Simply follow the on-screen prompts to:

1. Select options using numbers
2. Confirm destructive operations
3. Navigate back using option 0

## Safety Features

- Color-coded interface for better visibility
- Confirmation prompts for destructive operations
- Clear warnings before removing resources
- Graceful error handling

## Notes

- The tool searches for docker-compose files recursively from the current directory
- Project-specific cleanup attempts to identify resources based on docker-compose project names
- All operations require appropriate Docker permissions
